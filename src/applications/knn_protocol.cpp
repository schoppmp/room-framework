...
namespace sparse_linear_algebra::applications::knn {
  ...
  // Example:
  //    // optional: generate random documents for client and server respectively as normal distributed matrices.
  //    // IMPORTANT: At the moment, only a single client matrix is supported, which is reflected in this example.
  //    Eigen::SparseMatrix<T, Eigen::RowMajor> server_matrix(num_documents_server, num_words_in_vocabulary);
  //    Eigen::SparseMatrix<T, Eigen::ColMajor> client_matrix(num_words_in_vocabulary, 1);
  //    generateRandomMatrices(123456, precision, num_documents_server, m, n, num_nonzeros_server, num_nonzeros_client, &server_matrix, &client_matrix);
  //    // instantiate the protocol
  //    KNNProtocol protocol(channel, party_id, statistical_security, precision, 
  //        mt, pt, chunk_size, num_top_k, num_documents_server, num_words_in_vocabulary, 1, 
  //        num_nonzeros_server, num_nonzeros_client, server_matrix, client_matrix);
  //    // Run the protocol. Result are the indices for the server's top k documents 
  //    // that are most similiar to the client's document.
  //    auto result = protocol.run();

  KNNProtocol::KNNProtocol(comm_channel* channel, int party_id, int16_t statistical_security,
          ...
          size_t num_nonzeros_client, Eigen::SparseMatrix<T, Eigen::RowMajor> server_matrix,
      ...
      : channel(channel),
        ...
        precision(precision),
        mt(mt), pt(pt), chunk_size(chunk_size), k(k), num_documents_server(num_documents_server), num_words(num_words),
        num_documents_client(num_documents_client),
        num_nonzeros_server(num_nonzeros_server), num_nonzeros_client(num_nonzeros_client),
        server_matrix(server_matrix),
        client_matrix(client_matrix),
        result_matrix(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>(num_documents_server, num_documents_client))
  {
  ...
  void KNNProtocol::computeSimilarities() {
    for (int row = 0; row < num_documents_server; row += chunk_size) {
      ...
      switch (this->mt) {
        case dense: {
          fake_triple_provider<T> triples(dense_chunk_size, num_words, num_documents_client, party_id);
          channel->sync();
          ...
          benchmark(
              [&] {
                result_matrix.middleRows(row, this_chunk_size) = matrix_multiplication(
                    Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>(server_matrix)
                        .middleRows(row, this_chunk_size),
                    Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>(client_matrix), *channel,
                    party_id, triples, dense_chunk_size);
              },
              "Total");
          break;
        }
        case sparse: {
          fake_triple_provider<T> triples(dense_chunk_size, num_nonzeros_server + num_nonzeros_client, num_documents_client,
                                          party_id);
          channel->sync();
          benchmark(
              [&] {
              ...
              [&] {
                result_matrix.middleRows(row, this_chunk_size) =
                    matrix_multiplication_cols_rows(
                        server_matrix.middleRows(row, this_chunk_size), client_matrix, *pir_protocols[pt],
                        *channel, party_id, triples, dense_chunk_size, num_nonzeros_server, num_nonzeros_client,
                        true);
              },
              "Total");
          break;
  ...
  std::vector<int> KNNProtocol::topK() {
    ...
    for (int i = 0; i < num_documents_server; i++) {
      norms_A[i] = (server_matrix.row(i) / (static_cast<T>(1) << precision)).norm();
    }
    ...
    serialize_le(serialized_inputs.begin(), inputs_vec.begin(), num_documents_server);
    if (party_id == 0) {
      serialized_norms.resize(num_documents_server * sizeof(T));
      serialize_le(serialized_norms.begin(), norms_A.begin(), num_documents_server);
    } else {
      serialized_norms.resize(sizeof(T));
      serialize_le(serialized_norms.begin(), &norm_B, num_documents_client);
    }
    knn_oblivc_args args = {
        static_cast<int>(num_documents_server),      static_cast<int>(k),     sizeof(T),
        serialized_inputs.data(), serialized_norms.data(), outputs.data()};
