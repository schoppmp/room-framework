extern "C" {
  #include "src/applications/sigmoid.h"
}
#include "mpc_utils/mpc_config.hpp"
#include "mpc_utils/party.hpp"
#include "src/matrix_multiplication/dense.hpp"
#include "src/matrix_multiplication/sparse/cols-rows.hpp"
#include "src/matrix_multiplication/sparse/cols-dense.hpp"
#include "src/matrix_multiplication/sparse/rows-dense.hpp"
...
protected:
  void validate() {
    ...
    if(batch_size.size() == 0) {
      BOOST_THROW_EXCEPTION(po::error("'batch_size' must be passed at least once"));
    }
    ...
    if(nonzeros_a.size() == 0) {
      BOOST_THROW_EXCEPTION(po::error("'nonzeros_a' must be passed at least once"));
    }
    ...
    if(nonzeros_b.size() == 0) {
      BOOST_THROW_EXCEPTION(po::error("'nonzeros_b' must be passed at least once"));
    }
...
public:
  ...
  knn_config() : mpc_config() {
    ...
    add_options()
      ...
      ("batch_size", po::value(&batch_size)->composing(), "Number of documents to process at once; can be passed multiple times")
      ...
      ("nonzeros_a,a", po::value(&nonzeros_a)->composing(), "Number of non-zero columns in each batch of the first party's matrix A; can be passed multiple times")
      ("nonzeros_b,b", po::value(&nonzeros_b)->composing(), "Number of non-zero columns in each batch of the second party's matrix B; can be passed multiple times")
      ("num_epochs,e", po::value(&num_epochs)->composing(), "Number of epochs to train for. Defaults to 1. Can be passed multiple times")
...
int main(int argc, const char *argv[]) {
  ...
  size_t num_experiments = std::max({conf.num_documents.size(),
    conf.batch_size.size(), conf.vocabulary_size.size(),
    conf.nonzeros_a.size(), conf.nonzeros_b.size(), conf.num_epochs.size(),
      ...
      for (int epoch = 0; epoch < num_epochs; epoch++) {
        ...
        while (!done[0] && !done[1]) {
          ...
          if (row_index[active_party] + this_batch_size >
            ...
            this_batch_size = input[active_party]->rows() -
              row_index[active_party];
          }
