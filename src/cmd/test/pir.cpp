 ...
 public:
  ...
  test_pir_config() {
    ...
    add_options()("num_elements_server,N",
                  po::value(&num_elements_server)->required(),
                  "Number of non-zero elements in the server's database")(
        "num_elements_client,n", po::value(&num_elements_client)->required(),
        "Number of non-zero elements in the client's database")(
        "statistical_security,s",
        po::value(&statistical_security)->default_value(40),
        "Statistical security parameter")(
        "pir_type", po::value(&pir_type)->required(),
        "PIR type: basic | poly | fss | fss_cprg | scs");
...
int main(int argc, const char **argv) {
  ...
  try {
    ...
    if (party.get_id() == 0) {
      ...
      // run PIR protocol
      benchmark([&]() { proto->run_server(server_in, defaults); },
                "PIR Protocol (Server)");
      ...
      benchmark([&]() { proto->run_client(client_in, result); },
                "PIR Protocol (Client)");
      // run again with shared output
      benchmark([&]() { proto->run_client(client_in, result_shared, true); },
      ...
      for (size_t i = 0; i < result.size(); i++) {
        ...
        if (expected_shared != sum_shared) {
          std::cerr << "Input " << client_in[i] << ", expected "
                    << expected_shared << ", got " << sum_shared
                    << " (shared)\n";
          incorrect_output = true;
        }
      }
