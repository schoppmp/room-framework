...
public:
  ...
  zero_sharing_config() {
    namespace po = boost::program_options;
    add_options()
      ("num_nonzeros,k", po::value(&num_nonzeros)->required(), "Number of non-zero elements in the shared vector (= length of the input vector)")
      ("len,n", po::value(&len)->required(), "Number elements in the resulting vector");
