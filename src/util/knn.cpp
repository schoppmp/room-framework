...
namespace sparse_linear_algebra::util::knn {
  ...
  KNNConfig::KNNConfig():mpc_config() {
    ...
    add_options()(
        ...
        "multiple times")(
        "multiplication_type",
        po::value(&multiplication_types_raw)->composing(),
 ...
 void runExperiments(comm_channel* channel, int party_id, int16_t statistical_security,
        int precision, KNNConfig& conf) {
    using T = uint64_t;
  ...
  void generateRandomMatrices(int seed, int precision, size_t num_documents_client, size_t num_words, 
    size_t num_documents_server, size_t num_nonzeros_server, size_t num_nonzeros_client, 
    Eigen::SparseMatrix<T, Eigen::RowMajor>* server_matrix,  Eigen::SparseMatrix<T, Eigen::ColMajor>* client_matrix) {
    ...
    for (size_t j = 0; j < indices_A.size(); j++) {
      for (size_t i = 0; i < server_matrix->rows(); i++) {
        ...
        if (i == 23 && j < num_nonzeros_client) {
          // Set B to the first num_nonzeros_client nonzero elements of the 23rd document.
